package com.gu.xiongdilian.receiver;
...
public class MyMessageReceiver extends BroadcastReceiver {
    ...
    private BmobChatUser currentUser = null;
                        ...
                        BmobChatManager.getInstance(context).createReceiveMsg(json, new OnReceiveListener() {
                            ...
                            public void onSuccess(BmobMsg msg) {
                                if (ehList.size() > 0) {// 有监听的时候，传递下去
                                    ...
                                    boolean isAllow =
                                            (boolean) SPUtils.get(context, MyConfig.SHARED_KEY_NOTIFY, true);
                            ...
                            if (currentUser != null) {// 有登陆用户
                                if (toId.equals(currentUser.getObjectId())) {
                                    if (ehList.size() > 0) {// 有监听的时候，传递下去
                                        ...
                                        //TODO
                                        //showOtherNotify(context, message.getFromname(), toId, message.getFromname()
                                        //+ "请求添加好友", NewFriendActivity.class);
                                    }
                                }
                            }
                                    ...
                                    new FindListener<BmobChatUser>() {
                                        ...
                                        public void onSuccess(List<BmobChatUser> arg0) {
                                            ...
                                            XiongDiLianApplication.getXiongDiLianInstance()
                                                    .setContactList(CollectionUtils.list2map(BmobDB.create(context)
                                                            .getContactList()));
                                        }
                                    });
                            // TODO: 2015/9/11  
                            // 显示通知  
                            //showOtherNotify(context, username, toId, username + "同意添加您为好友", FriendMainActivity.class);
    ...
    public void showMsgNotify(Context context, BmobMsg msg) {
        ...
        boolean isAllowVoice = (boolean) SPUtils.get(context, MyConfig.SHARED_KEY_VOICE, true);
        boolean isAllowVibrate = (boolean) SPUtils.get(context, MyConfig.SHARED_KEY_VIBRATE, true);
    ...
    public void showOtherNotify(Context context, String username, String toId, String ticker, Class<?> cls) {
        boolean isAllow = (boolean) SPUtils.get(context, MyConfig.SHARED_KEY_NOTIFY, true);
        boolean isAllowVoice = (boolean) SPUtils.get(context, MyConfig.SHARED_KEY_VOICE, true);
        boolean isAllowVibrate = (boolean) SPUtils.get(context, MyConfig.SHARED_KEY_VIBRATE, true);
