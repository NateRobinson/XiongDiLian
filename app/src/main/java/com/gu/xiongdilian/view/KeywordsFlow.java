...
/**
 * @author nate
 * @ClassName: KeywordsFlow
 * @Description: 带飞入飞出效果的自定义布局
 * @date 2015-6-2 上午10:35:01
 */
public class KeywordsFlow extends FrameLayout implements OnGlobalLayoutListener {
    ...
    private boolean show() {
        if (width > 0 && height > 0 && vecKeywords != null && vecKeywords.size() > 0 && enableShow) {
            ...
            Log.d("ANDROID_LAB", "--------------------------width=" + width + " height=" + height + "  xItem=" + xItem
                    + " yItem=" + yItem + "---------------------------");
    ...
    public AnimationSet getAnimationSet(int[] xy, int xCenter, int yCenter, int type) {
        ...
        if (type == OUTSIDE_TO_LOCATION) {
            ...
            TranslateAnimation translate =
                    new TranslateAnimation((xy[IDX_X] + (xy[IDX_TXT_LENGTH] >> 1) - xCenter) << 1, 0,
            ...
            TranslateAnimation translate =
                    new TranslateAnimation(0, (xy[IDX_X] + (xy[IDX_TXT_LENGTH] >> 1) - xCenter) << 1, 0,
                            (xy[IDX_Y] - yCenter) << 1);
            animSet.addAnimation(translate);
