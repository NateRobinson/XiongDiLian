 ...
 * @author nate
 * @ClassName: MTextView
 * @Description: 解决TextView自动换行问题，可以插入图片表情，可以对某一段字加入背景
 * @date 2015-5-29 下午2:11:37
 */
public class MTextView extends TextView {
    ...
    public void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            textColor = array.getColor(R.styleable.MTextView_textColor, Color.BLACK);
            array.recycle();
        }
            ...
            for (int j = 0; j < aContentList.line.size(); j++) {
                ...
                if (ob instanceof String) {
                    ...
                    if (span instanceof DynamicDrawableSpan) {
                        ...
                        ((DynamicDrawableSpan) span).draw(canvas,
                                text,
                                start,
                                end,
                                (int) x,
                        ...
                        textBgColorRect.bottom =
                                (int) (height + aContentList.height + lineSpacing - mFontMetrics.descent);
                        canvas.drawRect(textBgColorRect, textBgColorPaint);
                        canvas.drawText(((SpanObject) ob).source.toString(), realDrawedWidth, height
                                + aContentList.height - mFontMetrics.descent, paint);
                        realDrawedWidth += width;
                    } else
                    // 做字符串处理
                    {
    ...
    private int measureContentHeight(int width) {
        ...
        for (int i = 0; i < obList.size(); i++) {
            ...
            if (width - drawedWidth < obWidth || splitFlag) {
                ...
                if (paragraphSpacing > 0 && objNum > 0 && line.line.get(objNum - 1) instanceof String
