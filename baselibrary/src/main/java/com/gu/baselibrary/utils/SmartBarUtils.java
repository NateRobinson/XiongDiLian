...
/**
 * 为魅族手机的smartbar设计的工具类，用来实现smart的展示和隐藏
 */
public class SmartBarUtils {
    ...
    /**
     * 调用 ActionBar.setTabsShowAtBottom(boolean) 方法。 如果 android:uiOptions="splitActionBarWhenNarrow"，则可设置ActionBar
     * Tabs显示在底栏。
     */
    public static void setActionBarTabsShowAtBottom(ActionBar actionbar, boolean showAtBottom) {
        try {
            Method method =
                    Class.forName("android.app.ActionBar").getMethod("setTabsShowAtBottom", new Class[]{boolean.class});
            try {
    ...
    /**
     * 调用 ActionBar.setActionBarViewCollapsable(boolean) 方法。 设置ActionBar顶栏无显示内容时是否隐藏。
     */
    public static void setActionBarViewCollapsable(ActionBar actionbar, boolean collapsable) {
        try {
            Method method =
                    Class.forName("android.app.ActionBar").getMethod("setActionBarViewCollapsable",
    ...
    public static void setActionModeHeaderHidden(ActionBar actionbar, boolean hidden) {
        try {
            Method method =
                    Class.forName("android.app.ActionBar").getMethod("setActionModeHeaderHidden",
                            new Class[]{boolean.class});
            try {
    ...
    /**
     * 调用ActionBar.setBackButtonDrawable(Drawable)方法设置返回键图标 示例： SmartBarUtils.setBackIcon(bar,
     * getResources().getDrawable(R.drawable.ic_back));
     */
    public static void setBackIcon(ActionBar actionbar, Drawable backIcon) {
        try {
            Method method =
                    Class.forName("android.app.ActionBar").getMethod("setBackButtonDrawable", new Class[]{Drawable.class});
            try {
    ...
    /**
     * 以下三个方法原作者为c跳跳(http://weibo.com/u/1698085875), 由Shawn(http://weibo.com/linshen2011)在其基础上改进了一种判断SmartBar是否存在的方法,
     * 注意该方法反射的接口只存在于2013年6月之后魅族的flyme固件中
     */
    /**
     * 方法一:uc等在使用的方法(新旧版flyme均有效)， 此方法需要配合requestWindowFeature(Window.FEATURE_NO_TITLE )使用,缺点是程序无法使用系统actionbar
     *
    ...
    /**
     * 方法二：此方法需要配合requestWindowFeature(Window.FEATURE_NO_TITLE)使用 ，缺点是程序无法使用系统actionbar
     *
     * @param context
     * @param window
     */
